...
int main() {
	...
	assert(testValidate_Valid_Object());
	assert(testValidate_Valid_Group());
	...
	assert(testValidate_Id("no", false));

	assert(testValidate_MissingType());
	assert(testValidate_Type(JsonPropertyValue::MapLayerTypeValue::TILE, true));
	assert(testValidate_Type(JsonPropertyValue::MapLayerTypeValue::OBJECT, true));
	assert(testValidate_Type(JsonPropertyValue::MapLayerTypeValue::GROUP, true));
	assert(testValidate_Type("idontknowyou", false));
	assert(testValidate_Type(5, false));

	assert(testValidate_MissingName());
	assert(testValidate_Name("Ground", true));
	assert(testValidate_Name(42, false));

	assert(testValidate_MissingWidth());
	assert(testValidate_Width(1, true));
	assert(testValidate_Width(0, false));
	assert(testValidate_Width("5", false));

	assert(testValidate_MissingHeight());
	assert(testValidate_Height(1, true));
	assert(testValidate_Height(0, false));
	assert(testValidate_Height("1", false));

	assert(testValidate_MissingData());
	assert(testValidate_InvalidData_NotArray(1));
	assert(testValidate_InvalidData_NotArray("huh"));
	assert(testValidate_InvalidData_Size());
	assert(testValidate_InvalidData_CellType());

	assert(testValidate_InvalidObjectList());
	...
	assert(testLocalizeValidationResult_DataInvalid());
	assert(testLocalizeValidationResult_ObjectListInvalid());
	...
	assert(testConvertToDefn_Group());
